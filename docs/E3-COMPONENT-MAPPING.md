...
| `cyberpunk/mappins/minimapMappins.reds` | Minimap mappin/clamping/police-state presentation. | Native minimap/mappin systems. | **In scope visually, native logically.** Keep route/POI/police data and behavior native while integrating the persistent navigation corner into the red/minimal HUD language. | `E3NavigationHudNative.reds`. | Same location: ON navigation/minimap area reads as part of the E3 HUD; OFF yields; route information is unchanged. |
| `cyberpunk/mappins/questMappins.reds` | Quest marker visual presentation. | Native quest mappin controllers/data. | **Do not recreate Project E3 marker logic.** Native quest marker semantics remain; the persistent navigation host carries the E3 visual language. | `E3NavigationHudNative.reds`. | Quest marker/routing remains correct in both modes. |
| `cyberpunk/quests/mappinsControllers.reds` | Empty in preserved 2.31.p2 reference. | N/A. | **No implementation required.** | None. | N/A. |
| `cyberpunk/widgets/compass/*` | Extensive archive-backed compass/POI/quest-marker system on `IronsightGameController`. | `IronsightGameController`, native navigation/mappin state. | **In scope only as neutral navigation presentation.** Do not import the historical compass subsystem, custom widget paths, optimizer, or mappin pools. Style the current native navigation host instead. | `E3NavigationHudNative.reds`. | ON navigation corner is red/minimal and coherent; current navigation capability is preserved. |
...
| Project E3 mappin/UI tweak files | Historical profile/icon/clamping changes. | Native mappin records/profiles. | **Reference only.** Biology does not import Project E3 TweakDB authority. | Native navigation host styling only. | Navigation semantics unchanged. |
...
- quest/objective tracker;
- navigation/minimap host presentation;
- weapon/ammo roster;
