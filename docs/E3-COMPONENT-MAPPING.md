...
| `cyberpunk/widgets/healthbar/nameplateVisuals.reds` | Projected identity visibility and E3 name/frame styling. | `NameplateVisualsLogicController`; native `NPCNextToTheCrosshair` identity/focus data; authored `m_nameTextMain` + `m_nameFrame`. | **Core in scope.** W20.1 keeps the already exact-compiled `SetVisualData(...)` wrapper call shape but passes an unmodified local copy of the native identity values. Public `GetDisplayName()` is presentation-only fallback after native visibility handling. Biology styles the authored native name text/frame directly; it no longer creates a fixed controller-root identity canvas. | Random civilian can show legitimate baseline identity without scanner; discovered police/combatant identity is never downgraded by Biology fallback; OFF returns native presentation. |
...
| Ambient identity lifecycle | Ordinary names are visibly live, but T005 exposed backward identity flow for police. | **W20.1 correction.** Preserve `SetElementVisibility` refresh, `IsAnyElementVisible` ambient-name gate, native projection and 10 / 20 ordinary range, but never enrich the data passed into native `SetVisualData`. Resolve public display-name fallback only for Biology presentation after native handling. |
| Nameplate visual design | T005 proved the fixed 340 x 46 Biology canvas can detach from the actual authored name. | **W20.1 correction.** Remove the parallel runtime canvas entirely. The authored native `m_nameTextMain` + `m_nameFrame` are the structural unit; Biology only applies reversible text/frame styling. If deeper structural fidelity is later required, use a Biology-owned authored REDmod/INK resource rather than controller-root rectangles. No health bar/number. |
...
| Quest/objectives | Right-hand `FOOL ON THE HILL` stack remains essentially current despite the quest controller hook. | `QuestTrackerGameController.m_questTrackerContainer` owns tracker visibility; `m_ObjectiveContainer` owns an iterated objective-only child list; `m_QuestTitle` is the native title. | Mount chrome only inside `m_questTrackerContainer`; never inject Biology children into `m_ObjectiveContainer`. Tint/restore `m_QuestTitle`, keep Journal/objective lifecycle native, and fail closed if the tracker host is not compound. |
...
## W20.1 — T005 nameplate structure and identity-authority correction

T005 on exact source `a5818db6596e335824d75f596fc8204cc419de4f` established two bounded failures:

- the fixed `CRBiologyE3IdentityChrome` 340 x 46 controller-root canvas could render detached from the actual native name;
- police discovery could regress toward generic `NC RESIDENT` after Biology supplied its ambient fallback to native `SetVisualData`.

W20.1 changes the authority model to:

`native NPCNextToTheCrosshair / discovered identity -> unmodified native values through SetVisualData -> native visibility/content -> Biology presentation fallback only when native name remains empty`.

Biology therefore does **not** write `GetDisplayName()` fallback into the data passed to native `SetVisualData`. The fallback remains available only to render an otherwise-empty ambient name after native handling. This prevents Biology's generic public display name from becoming an input to native/scanner identity state.
