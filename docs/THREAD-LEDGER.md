...
|---|---|---|---|---|---|---|
| **P01.2** | `[P01.2] PARENT — Integration Orchestrator` | Parent / integration orchestrator | **ACTIVE** | **IN-PROGRESS** | Issue #35; read `docs/handoffs/PARENT-P01.2.md` first | Successor to P01.1. W09, W10, and W11 are merged. The W11 attended read-only probe proved the preserved pre-W10 framework residue SAFE-TO-APPLY under its exact plan. P01.2 now owns the plan-bound cleanup, exact canonical-main compile/build/install/deploy, and attended acceptance cycle. |
...
| **W11.1** | `[W11.1] RELEASE — Legacy Framework Transition Cleanup` | Safely retire pre-W10 Mod Settings / ArchiveXL / RED4ext residue before the next attended candidate | **USABLE** | **MERGED** | Issue #64 closed; PR #65 merged; `agent/pre-w10-framework-transition-cleanup`; worker head `052888059bb2f1e75a64804494d302ced6b51e9f` | W11 implementation and evidence are complete. The successful read-only probe found 42 exact retirement candidates, no competing-consumer evidence, and all five redscript paths protected; the generated transition plan is SAFE-TO-APPLY. Parent owns actual cleanup execution and subsequent candidate acceptance. |
...
P01.2 parent
  ...
  +-- W11 / PR #65 MERGED; issue #64 CLOSED
  |     `-- exact preserved-install probe returned SAFE-TO-APPLY for 42 retired framework files
  |     `-- no competing consumer evidence; all five redscript paths protected
  |
  +-- current installed game is still the pre-W10 candidate until P01.2 executes the approved transition plan
  |
  +-- next parent gate: execute and verify the plan-bound legacy-framework cleanup, then exact-compile/build/install/deploy one canonical-main candidate
  ...
  +-- issue #66 tracks cross-cutting operator-bootstrap hardening; it is not an active W11 implementation dependency
```

No worker lane is currently active. Merged worker conversations remain context reserves only. P01.2 should create another fresh W## lane only for a materially new finding. Issue #66 remains a parent-tracked cross-cutting operator-contract follow-up until deliberately routed.
