...
`autonomous-t007-3` (diagnostics OFF). Tests are under `tests/`. Every production
...
| G-002, G-074, G-080–081: launcher OFF/ON | REDmod-owned Items.BiologyLauncherActivationMarker; policy accessors and all production hooks | ActivationGates, PlayerDisableContract; exhaustive hook audit pending | Saves remain inert; generic loaders preserved; no persistent Biology launcher | BROKEN until every hook and local activation transition audited |
| G-016, G-080: obvious safe one-download installer | Build-BiologyPackage, BiologyReleaseInstall.Core; exact-hash collision preflight | BiologyReleaseInstallSafety, InstallContract | Extract outside game; shared loader conflicts fail before mutation | Native EXE implemented; 31 ownership/upgrade/rollback checks + actual junction PASS; final artifact pending |
| G-080: uninstall, upgrade, same-version reinstall | BiologyUninstallCore/Program, BiologyPriorInstallTransition.Core, removal verifier | PlayerUninstaller (42 baseline checks), release/transition/removal safety tests | Receipt-bounded files; no recursive game/shared-root deletion; preserve saves/dependencies | Source PASS; actual final artifact lifecycle pending |
| G-082, G-090: exact compile, reproducible package, CI | Build-OwnedAcceptance, Compile-Profile, Build-BiologyPackage, .github/workflows/ci.yml | Run-CI, PowerShellSyntax, CompileGuard, ArtifactPolicy | Proprietary base read-only; final.redscripts excluded | 65-source exact compile PASS; fresh-shell build and reference-probe failures repaired; final CI/package pending |
| G-083, G-091–093: evidence and safe autonomous completion | Owner #152 handoff supersedes sole-agent/launch process restrictions for this run | Test ledger and exact reports | No save loading/editing; safe main-menu smoke authorized; no workers | In progress; never convert static proof into live acceptance |
...
- Exact compile `autonomous-t007-2` and `autonomous-t007-3`: PASS, 65 sources,
