...
Check ($source.Contains('!IsStringValid(resolved.name)')) 'Owned fallback overwrites a legitimate native focus name.'
...
Check ($source.Contains('resolved.name = puppet.GetDisplayName()')) 'Owned fallback does not use the public entity display name.'
...
Write-Host "PASS: $script:checks owned scanned-civilian nameplate seam checks; exact 2.31 script_ref boundary, stock identity permissions and renderer remain authoritative."
