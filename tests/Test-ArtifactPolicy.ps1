...
New-Item -ItemType Directory -Force -Path (Join-Path $settings 'realpass/provenance') | Out-Null
...
[IO.File]::WriteAllText((Join-Path $settings 'realpass/provenance/components.json'),'{"components":[{"id":"realpass-project-original"},{"id":"redscript"},{"id":"red4ext"},{"id":"archivexl"},{"id":"mod-settings"}]}')
...
Write-Host 'PASS: artifact policy accepts clean RealPass plus the constrained redscript/RED4ext/ArchiveXL/Mod Settings component set, and fails closed on forbidden paths, blocked components, unneeded frameworks, and game files.'
