...
# The installed archive tree is nested under archive\pc. WolvenKit's archive and
# unbundle directory arguments are non-recursive, so the probe must enumerate real
# .archive files rather than pass bare archive\pc and fail before INK lookup.
...
Check ($probe.Contains('`$archiveArguments.Add(`$archiveFile.FullName)'.Replace('`# T002 disproved the W02.3 assumption that RipperdocInventoryController's root owns
...
Write-Host "PASS: $script:checks W02.4 native-detail-region geometry checks."
))) 'W02.4 archive-list probe does not pass concrete archive files.'
Check ($probe.Contains('`$unbundleArguments.Add(`$archiveFile.FullName)'.Replace('`# T002 disproved the W02.3 assumption that RipperdocInventoryController's root owns
# authored content placement. The stock item region is the editable virtual-grid child.
Check ($followup.Contains('inkVirtualCompoundRef.Get(this.m_virtualGridContainer)')) 'W02.4 does not resolve the native Cyberware item-region widget.'
Check ($followup.Contains('public final func CRApplyBiologyDetailRegionLayout(target: ref<inkWidget>) -> Bool')) 'W02.4 native detail-region layout adapter is missing.'
...
Write-Host "PASS: $script:checks W02.4 native-detail-region geometry checks."
))) 'W02.4 unbundle probe does not pass concrete archive files.'
