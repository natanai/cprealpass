...
# live below archive\pc, so the probe enumerates concrete archives. ArchiveTask filters
# FileEntry.FileName; unresolved depot paths surface as <FNV1a64>.bin. The probe must
# therefore search by both readable path and the exact depot hash, then extract by hash.
...
Check ($probe.Contains("`$targetResourceHash = '13533725445430520621'")) 'W02.4 probe does not pin the FNV1a64 hash for the Ripperdoc fullscreen resource.'
Check ($probe.Contains('^13533725445430520621\\.bin
...
)) 'W02.4 archive search cannot find the resource when WolvenKit leaves its depot path unresolved.'
...
Check ($probe.Contains('`$matchingArchives.Count -ne 1'.Replace('`
# T002 disproved the W02.3 assumption that RipperdocInventoryController's root owns
# authored content placement. The stock item region is the editable virtual-grid child.
Check ($followup.Contains('inkVirtualCompoundRef.Get(this.m_virtualGridContainer)')) 'W02.4 does not resolve the native Cyberware item-region widget.'
Check ($followup.Contains('public final func CRApplyBiologyDetailRegionLayout(target: ref<inkWidget>) -> Bool')) 'W02.4 native detail-region layout adapter is missing.'
...
Write-Host "PASS: $script:checks W02.4 native-detail-region geometry checks."
))) 'W02.4 probe does not fail closed unless exactly one archive owns the target resource.'
Check ($probe.Contains("`$cli,'unbundle',`$sourceArchive.FullName,")) 'W02.4 extraction is not bound to the discovered source archive.'
Check ($probe.Contains("'--hash',`$targetResourceHash")) 'W02.4 extraction still depends on WolvenKit depot-path resolution instead of the exact resource hash.'
Check ($probe.Contains("NORMALIZED_EXTRACTED_RESOURCE=ripperdoc.inkwidget")) 'W02.4 probe does not normalize hash-named extraction before CR2W serialization.'
