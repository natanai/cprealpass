...
Require $builder 'listed-wolvenkit' 'Builder must record successful WolvenKit listing for Cyberpunk .archive reference containers.'
Require $builder ([regex]::Escape("'archive',$path,'--list','--regex','.*'")) 'Builder must use WolvenKit read-only archive listing rather than extracting selected Cyberpunk archives.'
Require $builder ([regex]::Escape("Acquire-ArchiveToolchain.ps1') -CacheRoot (Join-Path $LibraryPath '_tooling')")) 'Builder must obtain the verified reusable toolchain from the managed reference cache before archive inventory.'
Require $builder 'toolSha256' 'Reference archive inventory must record the verified WolvenKit CLI identity.'
