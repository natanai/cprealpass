...
Require $builder 'archive.*\$path.*--list.*--regex' 'Builder must use WolvenKit read-only archive listing rather than extracting selected Cyberpunk archives.'
Require $builder 'Acquire-ArchiveToolchain\.ps1.*CacheRoot.*LibraryPath.*_tooling' 'Builder must obtain the verified reusable toolchain from the managed reference cache before archive inventory.'
