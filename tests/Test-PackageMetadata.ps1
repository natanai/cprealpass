...
if ($readme -notmatch '(?i)model/source artifact' -or $readme -notmatch 'm1-owned-settings') { throw 'Package README does not distinguish this model-only artifact from the live owned settings profile.' }
...
# This artifact deliberately excludes native adapters. RealpassSettings is active in
# the deployable owned profile, but it does not belong in this model-only handoff.
if ($destinations.ContainsKey('r6/scripts/CyberpunkRealism/RealpassSettings.reds')) {
    throw 'Native Mod Settings adapter entered the model-only development source artifact.'
}
...
if (@($requirements | Where-Object { $_ -match '(?i)Mod Settings|ArchiveXL|RED4ext' }).Count -ne 0) {
    throw 'Model-only development artifact incorrectly inherited live runtime framework prerequisites.'
}
if ($distribution.components | Where-Object { $_.id -eq 'mod-settings' -and $_.status -ne 'allowed' }) {
    throw 'Live distribution contract no longer permits the accepted Mod Settings runtime.'
}

Write-Host "PASS: development package metadata matches $($manifest.version), $(@($manifest.files).Count) project-original model/source files, remains redscript-only by artifact scope, and clearly distinguishes itself from the constrained live settings runtime."
