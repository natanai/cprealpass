...
Check ($preferenceUi.Contains('CRMountBiologyE3PreferenceInNativeContent(host: ref<inkVerticalPanel>)')) 'E3 preference no longer mounts through a Biology-owned local content host.'
Check ($biology.Contains('this.CRMountBiologyE3PreferenceInNativeContent(this.crBiologyNativeContent);')) 'Biology shell does not place the E3 preference inside authored detail composition.'
Check (-not $preferenceUi.Contains('GetRootCompoundWidget')) 'E3 preference regressed to fullscreen-root placement.'
Check (-not $preferenceUi.Contains('inkEAnchor.TopRight')) 'E3 preference regressed to the live-proven clipped TopRight strategy.'
