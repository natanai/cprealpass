...
Check (-not $probe.Contains('Get-ChildItem -LiteralPath $scriptRoot -Recurse')) 'Presentation native probe regressed to a noisy full-tree symbol sweep instead of narrow W03.6 source contracts.'
foreach ($needle in @('MinimapContainerController','IronsightGameController','QuestTrackerGameController','m_questTrackerContainer','m_QuestTitle','m_ObjectiveContainer','QuestTrackerObjectiveLogicController','m_objectiveTitle','m_trackingIcon','m_trackingFrame','WeaponRosterGameController','m_onFootContainer','m_weaponAmmoWrapper','m_weaponName','m_weaponCurrentAmmo','m_weaponTotalAmmo','HotkeysWidgetController','m_dpadHintsPanel','m_consumables','m_gadgets','m_cyberware','gameuiCrosshairContainerController','gameuiCrosshairBaseGameController','OnCrosshairStateChange','OnState_Scanning','interactionWidgetGameController','activityLogEntryLogicController','NpcNameplateGameController','NameplateVisualsLogicController','OnInitialize','OnScreenProjectionUpdate','SetVisualData','SetElementVisibility','IsAnyElementVisible','OnUpdateInteraction','SetText','c_DisplayRangeNotAggressive','c_MaxDisplayRangeNotAggressive')) {
...
Check ($probe.Contains("'\bm_dpadHintsPanel\b'") -and $probe.Contains("'\bm_consumables\b'") -and $probe.Contains("'\bm_gadgets\b'") -and $probe.Contains("'\bm_cyberware\b'")) 'Presentation probe no longer verifies W03.6 hotkey/quickslot native content-region fields.'
Check ($probe.Contains("'\bclass\s+QuestTrackerObjectiveLogicController\b'") -and $probe.Contains("'\bm_objectiveTitle\b'") -and $probe.Contains("'\bm_trackingIcon\b'") -and $probe.Contains("'\bm_trackingFrame\b'")) 'Presentation probe no longer verifies W03.6 native quest objective-row content fields.'
Check ($probe.Contains("'\bfunction\s+SetVisualData\s*\('") -and $probe.Contains("'\bfunction\s+SetElementVisibility\s*\('") -and $probe.Contains("'\bfunction\s+IsAnyElementVisible\s*\('")) 'Presentation probe no longer verifies the exact nameplate data/visibility lifecycle W20.1 preserves.'
