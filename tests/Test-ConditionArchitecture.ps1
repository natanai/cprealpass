...
Check ($biologyNative.Contains('public final func SetActive(isActive: Bool) -> Void') -and $biologyNative.Contains('wrappedMethod(isActive)')) 'Biology does not follow the stock hub active/inactive lifecycle.'
Check ($biologyNative.Contains('if !isActive') -and $biologyNative.Contains('this.crBiologyPanel.SetVisible(false)') -and $biologyNative.Contains('this.CRBiologyHideItemRows()')) 'Biology can leak its root-level panel/item picker after the stock hub deactivates.'
