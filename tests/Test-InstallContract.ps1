...
foreach ($required in @('INSTALL.txt','UNINSTALL.txt','BIOLOGY-VERSION.txt','SHA256SUMS.txt','biology/build-manifest.json','biology/provenance.json','mods/Biology/info.json')) { if ($requiredRoot -notcontains $required) { throw "Required release file missing: $required" } }
