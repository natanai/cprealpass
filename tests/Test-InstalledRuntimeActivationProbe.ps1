...
Require $probe 'r6/logs/redscript_rCURRENT\.log' 'W13.1 probe must capture the canonical live REDscript current log.'
Require $probe 'Get-Content.*-Tail 500' 'W13.1 probe must bound live REDscript log capture.'
...
Require $probe 'BOUNDARY 2 — REDSCRIPT LIVE LOADER / COMPILE' 'W13.1 probe must classify the live REDscript boundary explicitly.'
...
Write-Host 'PASS: W13.1 installed-runtime activation evidence tooling is exact-head, zero-local-repo-safe, read-only against the installed game, candidate-revision-bound, receipt/hash-aware, captures current REDscript runtime evidence, and refuses to guess beyond the first proven broken boundary.'
