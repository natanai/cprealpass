...
# Source-level candidate-preparation contract.
...
Require $source 'Retained artifact SHA-256:' 'W12 bootstrap must report artifact SHA-256.'
Require $source 'Retained artifact bytes:' 'W12 bootstrap must report artifact size.'
...
Require $source 'KEEP UNTIL ATTENDED TEST' 'W12 bootstrap must label retained state.'
Require $source 'MAY DELETE IMMEDIATELY' 'W12 bootstrap must label disposable state.'
...
Write-Host 'PASS: W12 bootstrap preserves exit-0 stderr, accepts linked worktrees, supports exact cached-origin fallback and zero-repo acquisition, retains exact artifacts/evidence, and stops before launch.'
