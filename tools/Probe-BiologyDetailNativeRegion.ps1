...
function Add-JsonNeedlePaths([string]$Raw,[string[]]$Needles,[int]$MaxOccurrences = 12) {
    ...
    function Visit-JsonNode([object]$Node,[string]$Path,[string[]]$Ancestors) {
        ...
        if ($Node -is [string]) {
            foreach ($needle in $Needles) {
                if ($counts[$needle] -ge $MaxOccurrences) {
                    continue
                }
                if ($Node.IndexOf($needle,[StringComparison]::OrdinalIgnoreCase) -lt 0) {
                    continue
                }
        ...
        if ($Node -is [pscustomobject]) {
            ...
            if (-not [string]::IsNullOrWhiteSpace($summary)) {
                ...
                if ($nextAncestors.Count -gt 8) {
                    $nextAncestors = @($nextAncestors | Select-Object -Last 8)
                }
            }

            foreach ($property in $Node.PSObject.Properties) {
                Visit-JsonNode $property.Value ($Path + "['" + $property.Name.Replace("'","''") + "']") $nextAncestors
            }
            return
        }
        ...
        if ($Node -is [Collections.IDictionary]) {
            foreach ($key in $Node.Keys) {
                Visit-JsonNode $Node[$key] ($Path + "['" + ([string]$key).Replace("'","''") + "']") $Ancestors
            }
            return
        }
    ...
    Add-Report 'Purpose: expose redistribution-safe JSON paths/ancestor summaries for controller refs so the direct virtual-grid parent can be classified without committing the proprietary serialized resource.'
    Visit-JsonNode $root '
