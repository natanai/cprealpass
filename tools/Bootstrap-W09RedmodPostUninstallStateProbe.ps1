...
function Add-BinaryNeedleEvidence([string]$Label,[string]$Path,[string[]]$Needles) {
    Add-Evidence "=== $Label BOUNDED BINARY-STRING EVIDENCE ==="
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
        Add-Evidence "Executable absent: $Path"
        Add-Evidence ''
        return
    }
    ...
    foreach ($needle in $Needles) {
        ...
        $present = (Test-ByteSequence $bytes $ascii) -or (Test-ByteSequence $bytes $utf16)
        Add-Evidence ("Literal present [{0}]: {1}" -f $needle,$present)
    }
    Add-Evidence 'Interpretation: literal-string presence/absence is lexical evidence only; it does not prove a launcher or REDmod code path creates, deletes, or requires a directory.'
    Add-Evidence ''
}
...
try {
    ...
    $redmodRoot = Join-Path $GameRoot 'tools\redmod'
    ...
    if (Test-Path -LiteralPath $prelauncher -PathType Leaf) {
        ...
        Add-Evidence '=== OFFICIAL PRELAUNCHER FINGERPRINT ==='
        Add-Evidence ('REDprelauncher path: ' + $prelauncher)
        ...
        Add-Evidence 'REDprelauncher.exe was not present at the game root; no launcher binary fingerprint was recorded.'
        Add-Evidence ''
    }
    ...
    Add-Evidence '=== OFFICIAL REDMOD --HELP CHILD PROCESS ==='
    ...
    $needles = @('r6\cache\modded','r6/cache/modded','tweakdb_ep1.bin','mods.json')
    Add-BinaryNeedleEvidence -Label 'OFFICIAL REDMOD.EXE' -Path $redmodExe -Needles $needles
    if (Test-Path -LiteralPath $prelauncher -PathType Leaf) {
        Add-BinaryNeedleEvidence -Label 'OFFICIAL REDPRELAUNCHER.EXE' -Path $prelauncher -Needles $needles
    }
    ...
    Add-Evidence 'Executable hashes/versions, --help output, and bounded literal-string checks are direct official-tool evidence, but they do NOT prove historical causation or an internal directory-creation code path.'
    Add-Evidence 'This probe does NOT run REDmod deploy, does NOT invoke REDlauncher, does NOT test write permissions, and does NOT constitute official deployment/runtime acceptance.'
