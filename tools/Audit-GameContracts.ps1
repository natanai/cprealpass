...
if (-not $SkipCompile) {
    ...
    & (Join-Path $tools 'Build-OwnedAcceptance.ps1') -BuildId $compileBuildId -GameRoot $GamePath | Out-Null
...
Write-Host ''
Write-Host 'PASS: RealPass native-contract audit completed.' -ForegroundColor Green
Write-Host "Game version:            $gameVersion"
Write-Host "Base script SHA-256:     $baseBundleSha"
Write-Host "RealPass native hooks:   $($hooks.Count)"
