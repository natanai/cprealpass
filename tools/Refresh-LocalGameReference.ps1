...
if ($PSVersionTable.PSVersion.Major -lt 7) { throw 'Refresh-LocalGameReference.ps1 requires PowerShell 7 or newer.' }
...
if (-not (Test-Path -LiteralPath (Join-Path $RepoRoot '.git'))) {
    throw "Biology Git repository not found at: $RepoRoot"
}
...
Write-Host 'Game access is read-only. This tool writes only under reference\cyberpunk in the active Biology checkout.' -ForegroundColor DarkGray
...
$Environment | ConvertTo-Json -Depth 8 | Set-Content -Path (Join-Path $Out 'environment.json') -Encoding UTF8
    ...
    $Plugins = @(
        Get-ChildItem $PluginRoot -Directory -ErrorAction SilentlyContinue |
            ...
            ForEach-Object {
                ...
                [ordered]@{ Name = $PluginDir.Name; DLLs = $Dlls }
            }
    )
}
$Plugins | ConvertTo-Json -Depth 8 | Set-Content -Path (Join-Path $Out 'red4ext-plugins.json') -Encoding UTF8
...
$Frameworks | ConvertTo-Json -Depth 5 | Set-Content -Path (Join-Path $Out 'frameworks.json') -Encoding UTF8
...
Write-Host "Generated metadata : $Out" -ForegroundColor Cyan
Write-Host ''
