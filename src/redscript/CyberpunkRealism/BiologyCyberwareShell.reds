...
//
// The stock cyberware_equip fullscreen remains the technical screen because it
// already owns Cyberpunk's body silhouette, category anchors and zoom animations.
// realpass changes the hierarchy: the hub calls this BIOLOGY, Biology is the normal
// mode, and vanilla Cyberware remains available as an internal equipment mode.
...
// -----------------------------------------------------------------------------
// Hub label: keep stock routing/identifier, change only the visible parent label.
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Reuse stock Cyberware category anchors as Biology body-system nodes.
// Slot contents are hidden in Biology mode; the category label/position survives.
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Shared RipperDocGameController shell.
// -----------------------------------------------------------------------------
...
private final func CRCreateMetricRow(index: Int32) -> Void {
  ...
  background.SetTintColor(new HDRColor(0.70, 0.70, 0.70, 1.0));
  ...
  fill.SetTintColor(new HDRColor(0.85, 0.85, 0.85, 1.0));
  fill.Reparent(row, -1);
...
@addMethod(RipperDocGameController)
private final func CRRefreshBiologyOverview() -> Void {
...
@addMethod(RipperDocGameController)
private final func CRApplyBiologyShellMode(biology: Bool) -> Void {
