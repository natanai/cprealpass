...
private final func CRRefreshBiologyE3QuestFrame() -> Void {
  ...
  // Restore any Biology-owned style before sampling the native/current appearance.
  // This makes every ON cycle capture the current native value rather than a stale
  // value from an earlier HUD lifecycle.
...
private func UpdateTrackerData() -> Void {
  // Native tracker data can replace/re-style title and objective rows. Remove Biology's
  // previous pass first, let native update authoritatively, then apply current E3 state.
  this.CRRestoreBiologyE3QuestStyle();
  wrappedMethod();
  this.CRRefreshBiologyE3QuestFrame();
}
